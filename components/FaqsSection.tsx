    ...
    <div className="flex place-items-center">
      <div className="flex flex-col gap-6 my-4">
        <motion.div
          ...
          className="header text-center text-[24px]/[32px] font-[600] md:text-[40px]/[48px]"
        >
          {FAQCOMPONENT.title}
        </motion.div>
        ...
        <div className="faqs-ans flex flex-col lg:items-center  w-[340px] h-[472px] md:w-[712px] md:h-[384px] lg:w-[1200px] lg:h-[320px] divide-y divide-gray-400 ">
          {FAQCOMPONENT.questions.map((question, index: number) => {
            const answer = FAQCOMPONENT.answers.find(
            ...
            return (
              <div key={question.id}>
                <h1
                  key={index}
                  className="last:border-b flex justify-between items-center py-3 text-[16px]/[24px] md:text-[20px]/[28px] lg:w-[1142px]"
                  onClick={() => toggleDown(index)}
                >
                  {question.question}{" "}
                  <span className="block lg:hidden text-[24px]/[24px] md:text-[32px]/[32px]">
